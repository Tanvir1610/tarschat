...
export function ChatArea({ conversationId, currentUser, onBack }: ChatAreaProps) {
  ...
  useEffect(() => {
    if (isAtBottom) {
      ...
      if (lastMsg.senderId !== currentUser._id) setShowNewMessages(true);
    }
  }, [messages]);
  ...
  const isGroupAdmin =
    conversation?.type === "group" && conversation.adminId === currentUser._id;
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-200 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          ...
          {conversation?.type === "group" ? (
            ...
            <p className={`text-xs font-medium ${otherMember.isOnline ? "text-green-500" : "text-gray-400"}`}>
              {otherMember.isOnline ? "● Online" : "Offline"}
            </p>
          ) : null}
        </div>

        {/* Add members button for group admin */}
        {isGroupAdmin && (
          <button
            onClick={() => setShowAddMembers(true)}
            className="flex items-center gap-1.5 bg-blue-50 text-blue-600 text-xs px-3 py-1.5 rounded-full hover:bg-blue-100 transition-colors font-medium"
            title="Add members"
          >
          ...
          <div className="space-y-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className={`flex ${i % 2 === 0 ? "justify-start" : "justify-end"} animate-pulse`}>
                <div className={`rounded-2xl h-10 ${i % 2 === 0 ? "bg-white w-48" : "bg-blue-200 w-36"}`} />
              </div>
            ))}
          </div>
        ) : messages.length === 0 ? (
